...
import { defaultNote, defaultItem } from './../../utilities/DefaultObject';
...
const OutWarehouse = () => {
    ...
    const handleNoteInputChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setNote(updatedNote);
        if (name === "wareHouseId") {
            ProductServices.GetProductInWareHouses(parseInt(value)).then((res) => {
                console.log(res);
                if (res.isSuccess) {
                    setProductList(res.result.products || [])
                }
            });
            // Đặt danh sách sản phẩm về rỗng
            setItemList([]);
            setProductAddedList([]);
            // Đặt ghi chú về trạng thái ban đầu với items là một mảng rỗng
            setNote(prevNote => ({
                ...prevNote,
                noteItems: []
            }));
    ...
    const handleAddToItemsAndProductAddedList = (product: IProduct) => {
        ...
        setCustomerIdError('');
        setWarehouseIdError('');
        if (product.quantityInWareHouse <= 0) {
            alert(`Không thể thêm ${product.name} vì không còn tồn kho`)
        } else {
    ...
    const handleQuantityChange = (
        ...
        let updatedProductAddedList = [...productAddedList];
        let updatedItemList = [...itemList];

        if (newQuantity > quantityInWareHouse) {
            // Cập nhật số lượng của sản phẩm trong productAddedList
            updatedProductAddedList = updatedProductAddedList.map(product => {
                if (product.id === productId) {
                    return { ...product, quantity: quantityInWareHouse };
                }
                return product;
            });
            ...
            updatedItemList = updatedItemList.map(item => {
                if (item.productId === productId) {
                    const totalPrice = quantityInWareHouse * item.price; // Tính toán tổng thành tiền
                    return { ...item, quantity: quantityInWareHouse, totalPrice: totalPrice };
                }
                return item;
            });
        } else {
    ...
    const handlePriceChange = (
        ...
        // Cập nhật giá của sản phẩm trong productAddedList
        ...
        // Cập nhật giá của sản phẩm trong itemList
        const updatedItemList = itemList.map(item => {
            if (item.productId === productId) {
                const totalPrice = item.quantity * newPrice; // Tính toán tổng thành tiền
                return { ...item, price: newPrice, totalPrice: totalPrice };
            }
            return item;
        });
        ...
        <div className='border border-black p-3 bg-inout-green'>
            <div className=''>
                <h3 className='m-0 fw-bold'>
                    {isAdd ? "Thêm phiếu xuất kho" : "Cập nhật phiếu xuất kho"}
                    {!isAdd ?
                        ...
                        <table className="table table-sticky">
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-center border-black fw-bold bg-body-secondary">Tồn</th>
                            ...
                            <tbody>
                                {productAddedList && productAddedList.map((p) => (
                                    <tr key={p.id}>
                                        ...
                                        <td className="text-center border-black">{p.quantityInWareHouse}</td>
                                        {/* Số lượng */}
                                        <td className="text-center border-black">
                                            <input
                                                ...
                                                min={0}
                                                value={p.quantity || ''}
                                                onChange={(e) => handleQuantityChange(e, p.id, p.quantityInWareHouse)}
                                            />
                                        </td>
                                        {/* Đơn giá */}
                                        <td className="text-center border-black">
                                            <input
                                                ...
                                                min={0}
                                                value={p.price || 0}
                                                onChange={(e) => handlePriceChange(e, p.id, p.quantityInWareHouse)}
                                            />
                                        </td>
